...
import "package:community_testing_ryusdv/backend/schema/structs/index.dart" as community_testing_ryusdv_data_schema;
import "package:utility_functions_library_8g4bud/backend/schema/structs/index.dart" as utility_functions_library_8g4bud_data_schema;
import "package:shadcn_u_i_kit_v48jv9/backend/schema/structs/index.dart" as shadcn_u_i_kit_v48jv9_data_schema;import '/backend/schema/structs/index.dart';
import '/backend/supabase/supabase.dart';
import "package:community_testing_ryusdv/backend/schema/structs/index.dart" as community_testing_ryusdv_data_schema;
import "package:utility_functions_library_8g4bud/backend/schema/structs/index.dart" as utility_functions_library_8g4bud_data_schema;
import "package:shadcn_u_i_kit_v48jv9/backend/schema/structs/index.dart" as shadcn_u_i_kit_v48jv9_data_schema;
import "package:community_testing_ryusdv/backend/schema/enums/enums.dart" as community_testing_ryusdv_enums;
import "package:shadcn_u_i_kit_v48jv9/backend/schema/enums/enums.dart" as shadcn_u_i_kit_v48jv9_enums;
...
class SupabaseGoogleMapCustom extends StatefulWidget {
  final double? width;
  final double? height;
  ...
  @override
  _SupabaseGoogleMapCustomState createState() =>
      _SupabaseGoogleMapCustomState();
}

class _SupabaseGoogleMapCustomState extends State<SupabaseGoogleMapCustom> {
  // Supabse
  ...
  // Markers e Polygon and MapStyle
  Set<gmaps.Marker> _markers = {};
  Set<gmaps.Polygon> _polygons = {};
  String _mapStyle = MapStyle().dark;
  ...
  // Adicionando os Markers no Mapa
  Future<void> _fetchMarkers(BuildContext context) async {
    ...
    Map<String, gmaps.LatLng> adjustedPositions = {};

    response.forEach((item) async {
      double lat = item['latitude'];
      double lng = item['longitude'];

      // deslocamento de coordenadas iguais de markers
      ...
      gmaps.LatLng adjustedPosition = gmaps.LatLng(lat, lng);
      ...
      _addMarkers = gmaps.Marker(
        markerId: gmaps.MarkerId(item['membro_id']),
        position: adjustedPosition, //gmaps.LatLng(lat, lng),
        icon: markerIcon,
  ...
  // Adicionando os Poligonos no mapa
  Future<void> _fetchPolygons(BuildContext context) async {
    ...
    for (var item in response) {
      if (item['coordenadas'] == null ||
          item['coordenadas'] is! List ||
          !item['visivel']) continue;

      List<gmaps.LatLng> pontos = (item['coordenadas'] as List)
          .map((coord) => gmaps.LatLng(coord['lat'], coord['lng']))
          .toList();

      // cor da linha
      Color strokeColor =
          _hexToColor(item['cor_hex'] ?? _hexToColor("#0000FF"));
  ...
  // IMPLEMENMTAÇÕES ESPECIAIS NO MAPA GOOGLE
  // - Mudança de temas
  // - Recursos do Google Mapa

  // Temas
  Widget _buildThemes() {
    return Positioned(
      top: 16, // Ajustado para ficar mais próximo da borda superior
      right: 16,
      child: Column(
        children: [
                  ...
                  child: GridView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return GestureDetector(
                        onTap: () {
                          setState(() {
                            _mapStyle = _mapThemes[index]['style'];
                          });
